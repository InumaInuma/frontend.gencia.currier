...
export const MotorizadoEntregasPage: React.FC = () => {
  ...
  const handleConfirmarEntrega = async (fotoClienteFile: File | null, fotoPagoFile: File | null) => {
    ...
    try {
      await actualizarEstadoMutation.mutateAsync({
        ...
        fotoPruebaEntrega: fotoClienteFile,
        fotoComprobantePago: fotoPagoFile,
      });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex overflow-x-hidden w-full">
      {/* Left Sidebar */}
      <LeftSidebar
        contraido={contraido}
